document.addEventListener("DOMContentLoaded", () => {
	...
	const frames = document.querySelectorAll(".Drag");
    ...
    // load on open
	document.addEventListener("DOMContentLoaded", loadWindowState);
	...
	const scrollText = (text, speed = 200) => {
		let textMatrix = Array(5)
			.fill()
			.map(() => []); // Initialiser une matrice vide 5xN (où N > 8 pour tenir tout le texte)
	...
	const updateMatrix = (matrix) => {
		for (let row = 0; row < 5; row++) {
			// Utilisation des lignes du padLayout
			for (let col = 0; col < 8; col++) {
	...
	const initVelocityToggle = () => {
		velocityToggleButton.addEventListener("click", () => {
			...
			velocityToggleButton.textContent = velocityEnabled
				? "Désactiver Vélocité"
				: "Activer Vélocité";
		});
	};
	...
	const playNote = (frequency, velocity) => {
		...
		const attackTime = parseFloat(
			document.getElementById("attack-range").value
		);
		const decayTime = parseFloat(document.getElementById("decay-range").value);
		const sustainLevel =
			parseFloat(document.getElementById("sustain-range").value) * volume;
		const releaseTime = parseFloat(
			document.getElementById("release-range").value
		);

		// Récupérer les valeurs du LFO depuis les contrôles
		const vibratoFreq = parseFloat(
			document.getElementById("vibrato-frequency").value
		);
		const vibratoDepth = parseFloat(
			document.getElementById("vibrato-depth").value
		);

		// Récupérer les valeurs du filtre depuis les contrôles
		const filterFrequency = parseFloat(
	...
	const updateMidiPorts = () => {
		...
		availableOutputs = Array.from(midiAccess.outputs.values()).map(
			(output) => ({
				id: output.id,
				name: output.name,
			})
	...
	function makeDraggable(element) {
        ...
        function moveElement(e) {
            ...
            const positions = [
                { x: 0, y: 0 },
                { x: windowWidth - element.offsetWidth, y: 0 },
                { x: 0, y: windowHeight - element.offsetHeight },
                {
            ...
            for (let pos of positions) {
                if (
                    Math.abs(x - pos.x) < dockThreshold &&
                    Math.abs(y - pos.y) < dockThreshold
                ) {
                    dockedPosition = pos;
                    break;
                }
            ...
            if (dockedPosition) {
                ...
                frames.forEach((otherFrame) => {
                    if (otherFrame !== element && checkOverlap(element, otherFrame)) {
                        element.style.left = rect1.left + 10 + "px"; // Décalage en cas de chevauchement
                        element.style.top = rect1.top + 10 + "px";
                    }
                });
            }
        }
        ...
        function checkOverlap(element, otherElement) {
    ...
    function saveWindowState() {
			const state = Array.from(frames).map((frame) => {
				return {
					id: frame.id,
					...
					docked: frame.classList.contains("docked"),
				};
			});
	...
	function loadWindowState() {
			...
			if (state) {
				state.forEach((win) => {
					const frame = document.getElementById(win.id);
					if (frame) {
						frame.style.left = win.left;
						frame.style.top = win.top;
						frame.style.width = win.width;
						frame.style.height = win.height;
						if (win.docked) {
